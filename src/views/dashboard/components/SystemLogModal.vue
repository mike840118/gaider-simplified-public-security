  ...
  <div v-if="isOpen" class="modal-overlay" @click.self="closeModal">
    <div class="modal-content log-modal">
      <div class="modal-header">
        <h2 class="modal-title">{{ $t('system_log.title') }}</h2>
  ...
  patients.forEach(p => {
    ...
    if (p.sos) {
      generatedLogs.push({
        ...
        action: t('system_log.action.sos_alert')
      })
    }
    ...
    if (p.electricity < 30) {
      generatedLogs.push({
        ...
        time: timeStr,
        name: p.name || t('common.unknown'),
        action: t('system_log.action.low_battery')
      })
    }
    ...
    if (p.level === 'NORMAL') {
      generatedLogs.push({
        ...
        time: timeStr,
        name: p.name || t('common.unknown'),
        action: t('system_log.action.upload_normal')
      })
    } else {
      generatedLogs.push({
        rawTime: dateObj.getTime(),
        time: timeStr,
        name: p.name || t('common.unknown'),
        action: t('system_log.action.upload_abnormal')
      })
    }
  })
